...
LEFT JOIN [Boardgames]
 ...
 --8.	First 5 Boardgames
  ...
  SELECT TOP (5)
   ...
   WHERE [b].[Rating] > '7.00' AND CHARINDEX('a', LOWER([b].[Name])) > 0
...
--11.	Creator with Boardgames
     ...
     CREATE
         OR
      ALTER 
   FUNCTION [udf_CreatorWithBoardgames](@name NVARCHAR(30))
RETURNS INT
         AS
      BEGIN
...
--12.	Search for Boardgame with Specific Category
   ...
   CREATE
       OR
    ALTER
PROCEDURE [usp_SearchByCategory](@category VARCHAR(50))  
       AS
    BEGIN
           SELECT [b].[Name],
                  [b].[YearPublished],
