...
namespace MusicLibrary
{
    public class StartUp
    {
        public static void Main(string[] args)
        {
            ...
            //Try to add a track beyond capacity
            library.AddTrack(track7); //Should be skipped (capacity reached)
            ...
            //Try to remove not existing track
            bool removed = library.RemoveTrack("Enter Sandman", "Metallica");
            Console.WriteLine($"Track removed: {removed}"); //Output: Track removed: False

            //Try to remove existing track
            removed = library.RemoveTrack("Billie Jean", "Michael Jackson");
            Console.WriteLine($"Track removed: {removed}"); //Output: Track removed: True

            //Try to add a duplicate track
            library.AddTrack(track6); //Should be skipped (duplicate track)

            Console.WriteLine($"Track count after trying to add a duplicate track: {library.GetTracksCount()}");
            //Output: Track count after trying to add a duplicate track: 4
